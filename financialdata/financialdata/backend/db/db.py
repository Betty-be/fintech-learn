...
def build_dataframe_update_to_sql(table: str, df: pd.DataFrame) -> typing.List[str]:
...
def update_data_to_mysql_by_sql(
    ...
    commit(sql=sql, mysql_connection=mysql_connection)
...
def commit(
    ...
    try:
        ...
        if isinstance(sql, list):
            for s in sql:
                try:
                    mysql_connection.execution_options(autocommit=False).execute(s)
                except Exception as e:
                    logger.info(e)
                    logger.info(s)
                    break
            ...
            mysql_connection.execution_options(autocommit=False).execute(sql)
        trans.commit()
    except Exception as e:
        trans.rollback()
        logger.info(e)
...
def upload_data(
    data: pd.DataFrame, table: str, my_sql_connection: engine.base.Connection
) -> None:
    if len(data) > 0:
